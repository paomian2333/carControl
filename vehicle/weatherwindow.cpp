...
void weatherwindow::replyFinished(QNetworkReply *arg) // 对返回的数据进行json解析处理
{
    ...
    // updatetime
    // QJsonValue value_upt = obj2.take("updateTime");//取出cityInfo里的updateTime的值
    // qDebug()<<value_upt.toString();
    // ui->updatetime->setText("更新时间 "+value_upt.toString());//将updateTime的值赋给updatetime
...
QString weatherwindow::getCityCode(const QString& cityName, const QString& filePath)
{
    ...
    // Assuming the top-level structure is an object with a key "城市代码" containing an array
    ...
    if (rootObject.contains("城市代码"))
    {
        ...
        // Iterate through the array and search for the city name
        for (const QJsonValue& provinceValue : cityCodeArray)
        {
            ...
            for (const QJsonValue& cityValue : cityArray)
            {
                ...
                // Check if the current city name matches the desired city name
    ...
    // If the city name is not found, return an empty string
    return QString();
}
